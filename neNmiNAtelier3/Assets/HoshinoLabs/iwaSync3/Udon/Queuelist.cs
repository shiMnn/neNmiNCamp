    ...
    public class Queuelist : VideoControllerEventListener
    {
        ...
        private void Update()
        {
            ...
            else
            {
                ...
                {
                    var time = core.time;
                    _progressSlider.SetValueWithoutNotify(time / duration);
                }
            }
        }
        ...
        void PlayTrack()
        {
            ...
            core.TakeOwnership();
            core.PlayWithoutQueueURL(_modes[_track], _urls[_track]);
        ...
        public void AppendURL(uint mode, VRCUrl url) {
            if (!core.IsValidURL(url.Get())) {
                return;
            }
            TakeOwnership();
            _local = true;
            _on = true;
            _modes = _ArrayUtility_Add_uintarray_uint(_modes, mode);
            _urls = _ArrayUtility_Add_VRCUrlarray_VRCUrl(_urls, url);
            _local = false;
            _on = false;
            if (!core.isPrepared && !core.isPlaying && !core.isError && 0 < _urls.Length) {
                PlayTracks(_urls.Length - 1);
            }
            RequestSerialization();
            ValidateView();
        }
