...
namespace HoshinoLabs.IwaSync3.Udon
{
    ...
    public class VideoCore : UdonSharpBehaviour
    {
        ...
        public Queuelist queuelist => _queuelist;

        MaterialPropertyBlock _properties = null;
        RenderTexture _bufferTexture;
        ...
        private void Start()
        {
            ...
            _bufferTexture = new RenderTexture(256, 256, 0, RenderTextureFormat.ARGB32);
            ...
            _started = true;
            if (_url != VRCUrl.Empty && IsValidURL(_url.Get())) {
                SendCustomEventDelayedFrames(nameof(DelayedLoadURL), 0);
            }
        }

        private void LateUpdate() {
            if (isPlaying && isModeStream) {
                var mainTexture = _renderer.material.GetTexture("_MainTex");
                if (mainTexture != null) {
                    if (!_bufferTexture.IsCreated()) {
                        _bufferTexture.width = mainTexture.width;
                        _bufferTexture.height = mainTexture.height;
                        _bufferTexture.Create();
                    }
                    VRCGraphics.Blit(mainTexture, _bufferTexture);
                }
            }
        }
        ...
        public void RemoveListener(VideoCoreEventListener listener) {
            if (_listeners == null) {
                _listeners = new VideoCoreEventListener[0];
            }
            var idx = Array.IndexOf(_listeners, listener);
            if (idx < 0) {
                return;
            }
            var array = new VideoCoreEventListener[_listeners.Length - 1];
            Array.Copy(_listeners, 0, array, 0, idx);
            Array.Copy(_listeners, idx + 1, array, idx, _listeners.Length - idx - 1);
            _listeners = array;
        }
        #endregion
        ...
        public override void OnVideoError(VideoError videoError)
        {
            ...
            _status = _status | _status_error;
            // fallback
            if (videoError == VideoError.PlayerError && isModeVideo) {
                Debug.Log($"[<color=#47F1FF>{IwaSync3.APP_NAME}</color>] Fallback to Live to VRChat bugs.");
                SendCustomEventDelayedFrames(nameof(FallbackVideoMode), 0);
                return;
            }
            // error
            _error = videoError;
        ...
        /// <summary>
        /// 動画の再生をリクエストします。
        /// キューリストを連動している場合はキューリストに転送します。
        /// </summary>
        /// <param name="mode"></param>
        /// <param name="url"></param>
        public void PlayURL(uint mode, VRCUrl url) {
            if (_queuelist != null) {
                _queuelist.AppendURL(mode, url);
                return;
            }
            PlayWithoutQueueURL(mode, url);
        }
        ...
        /// <param name="mode"></param>
        /// <param name="url"></param>
        public void PlayWithoutQueueURL(uint mode, VRCUrl url) {
            if (_emitting) {
                return;
            }
            _mode = mode;
        ...
        public float brightness {
            ...
            set {
                defaultBrightness = Mathf.Clamp(value, 0f, 1f);
                UpdateBrightness();
            }
        }
