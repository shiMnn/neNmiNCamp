    ...
    internal class IwaSync3BuildProcessor : IProcessSceneWithReport
    {
        ...
        T[] FindObjectsOfType<T>(bool includeInactive) where T : Component
        {
            return Resources.FindObjectsOfTypeAll<T>()
                ...
                .Where(x => !IsEditorOnlyInHierarchy(x.transform))
                .Where(x => includeInactive || x.gameObject.activeInHierarchy)
                .ToArray();
        }

        bool IsEditorOnlyInHierarchy(Transform transform) {
            return transform != null && (transform.CompareTag("EditorOnly") || IsEditorOnlyInHierarchy(transform.parent));
        }
